...
```ts
  ...
  4、css特殊处理：Vite利用浏览器原生的ES模块加载机制，将CSS文件转化为Javascript模块，然后通过Javascript模块的方式将CSS动态注入到HTML也面，这样避免了对CSS的打包和编译加速了构建。
```

**题目35 setState是同步还是异步，如何实现同步？异步的原理是什么？**：
```ts
