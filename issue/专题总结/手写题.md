...
```JavaScript
  ...
  // 节流 - ts类型约束版
  function throttle<T extends (...args: any[] => any)>(fn: T, delay) {
    let timer: number | null = null // 定时器的返回值类型是number
    let result: RetuenType<T> | undefined // 节流返回函数的出参类型，由传入的fn推导
    return function (...arg: Parameters<T>) { // 节流返回函数的入参类型，由传入的fn推导
      let context = this
      if (!timer) { // 有定时器就不执行
        result = fn && fn.apply(context, arg)
        timer = setTimeout(() => {
          timer = null // 节流函数执行完需清空定时器
        }, delay)
        return result
